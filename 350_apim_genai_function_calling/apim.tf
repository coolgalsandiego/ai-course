...
resource "azapi_resource" "apim" {
  ...
  name                      = "apim-genai-basicv2-${random_string.random.result}-${var.prefix}"
  parent_id                 = azurerm_resource_group.rg.id
  location                  = azurerm_resource_group.rg.location # SKU BasicV2 is not supported in the region Sweden Central
